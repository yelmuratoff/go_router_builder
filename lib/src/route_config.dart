...
class ShellRouteConfig extends RouteBaseConfig {
  ...
  @override
  String get routeConstructorParameters => '${navigatorKey == null ? '' : 'navigatorKey: $navigatorKey,'}'
  ...
  String get factorConstructorParameters =>
      'builder: (context, state) => $_extensionName._fromState(state).build(context, state),';
...
class StatefulShellBranchConfig extends RouteBaseConfig {
  ...
  @override
  String get routeDataClassName => 'StatefulShellBranch';
...
class GoRouteConfig extends RouteBaseConfig {
  ...
  late final Set<String> _pathParams = pathParametersFromPattern(_rawJoinedPath);
  ...
  String get _locationArgs {
    final Map<String, String> pathParameters = Map<String, String>.fromEntries(
      _pathParams.map((String pathParameter) {
        ...
        final String value = '\${Uri.encodeComponent(${_encodeFor(pathParameter)}${type?.isEnum ?? false ? '!' : ''})}';
  ...
  ParameterElement? get _extraParam =>
  ...
  String _decodeFor(ParameterElement element) {
    if (element.isRequired) {
      if (element.type.nullabilitySuffix == NullabilitySuffix.question && _pathParams.contains(element.name)) {
        throw InvalidGenerationSourceError(
  ...
  late final List<ParameterElement> _ctorParams = _ctor.parameters.where((ParameterElement element) {
  ...
  late final List<ParameterElement> _ctorQueryParams = _ctor.parameters
      .where((ParameterElement element) => !_pathParams.contains(element.name) && !element.isExtraField)
      .toList();
...
''';
  ...
  Iterable<String> _enumDeclarations() {
    ...
    for (final ParameterElement ctorParam in <ParameterElement>[
      ...
      if (potentialEnumType is ParameterizedType && (ctorParam.type as ParameterizedType).typeArguments.isNotEmpty) {
...
''';
  ...
  @override
  String get routeDataClassName => 'GoRoute';
...
abstract class RouteBaseConfig {
  ...
  factory RouteBaseConfig.fromAnnotation(
    ...
    final RouteBaseConfig definition = RouteBaseConfig._fromAnnotation(reader, element, null);
  ...
  factory RouteBaseConfig._fromAnnotation(
    ...
    value._children.addAll(reader
        ...
        .listValue
        .map<RouteBaseConfig>((DartObject e) => RouteBaseConfig._fromAnnotation(ConstantReader(e), element, value)));
    ...
    final String? fieldDisplayName = classElement.fields
        .where((FieldElement element) {
          ...
          if (parameterName.toLowerCase().contains(RegExp('navigatorKey | observers'))) {
            ...
            if (typeArgument.getDisplayString(withNullability: false) != 'NavigatorState') {
              return false;
            }
          }
          return true;
  ...
  late final String _routeGetterName = r'$' + _className.substring(0, 1).toLowerCase() + _className.substring(1);
...
String _enumMapConst(InterfaceType type) {
  ...
  for (final FieldElement enumField in type.element.fields.where((FieldElement element) => element.isEnumConstant)) {
    buffer.writeln(
